    ...
    for(i=0; ; i++){
        ...
        if(ret>=0){
            ...
            if(ret>=0){
                ...
                printf(" st:%2d dts:%"PRId64" pts:%"PRId64" pos:%" PRId64 " size:%d flags:%d", pkt.stream_index, pkt.dts/**av_q2d(st->time_base)*/, pkt.pts/**av_q2d(st->time_base)*/, pkt.pos, pkt.size, pkt.flags);
            }
            printf("\n");
        }
        ...
        printf("ret:%2d st:%2d ts:%"PRId64" flags:%d\n", ret, stream_id, stream_id < 0 ? av_rescale(timestamp, st->time_base.den, AV_TIME_BASE * (int64_t)st->time_base.num) : timestamp, i&1);
    }
